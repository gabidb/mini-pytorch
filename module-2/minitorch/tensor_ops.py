    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        for i in range(len(a_storage)):
            a_index = np.zeros(len(a_shape), dtype=np.int32)
            broadcasted_index = np.zeros(len(out_shape), dtype=np.int32)
            to_index(i, a_shape, a_index)
            broadcast_index(a_index, a_shape, out_shape, broadcasted_index)
            position = index_to_position(a_index, a_strides)
            out_position = index_to_position(broadcasted_index, out_strides)
            out[out_position] = fn(out[out_position], a_storage[position])
