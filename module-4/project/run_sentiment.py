...
import embeddings
import sys
sys.path.insert(0, '/Users/georgievaga/Documents/Git/MLE/mle-module-4-93c3173d-gabidb')
import minitorch
...
class CNNSentimentKim(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        x1 = self.conv1d1.forward(embeddings).relu()
        x2 = self.conv1d2.forward(embeddings).relu()
        x3 = self.conv1d3.forward(embeddings).relu()

        x1 = minitorch.max(x1, dim=2)
        x2 = minitorch.max(x2, dim=2)
        x3 = minitorch.max(x3, dim=2)
