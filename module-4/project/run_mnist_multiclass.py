...
class Network(minitorch.Module):
    ...
    def forward(self, x):
        # TODO: Implement for Task 4.5.
        x1 = self.mid.forward(x).relu()
        x2 = self.out.forward(x1).relu()

        x3 = minitorch.maxpool2d(x2, (4, 4))
        x3 = x3.view(BATCH, 392)
        x3 = self.linear1.forward(x3).relu()
        x3 = self.linear2.forward(x3)
        return minitorch.logsoftmax(x3, dim=1)
