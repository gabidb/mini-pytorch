...
def tile(input: Tensor, kernel: Tuple[int, int]) -> Tuple[Tensor, int, int]:
    ...
    tiled = input.contiguous().view(batch, channel, new_height, kh, new_width, kw)\
        .permute(0, 1, 2, 4, 3, 5).contiguous().view(batch, channel, new_height, new_width, last_dim)
...
def logsoftmax(input: Tensor, dim: int) -> Tensor:
    ...
    # TODO: Implement for Task 4.4.
    t = max(input, dim)
    return input - t - (input - t).exp().sum(dim=dim).log()
...
def maxpool2d(input: Tensor, kernel: Tuple[int, int]) -> Tensor:
    ...
    out = max(input, dim=-1)
    return out.view(batch, channel, new_width, new_height)
